    ...
    <div class="seller-wrapper">
      ...
      <div class="seller-view">
        <div class="address-wrapper">
          <div class="address-left">{{sellerData.address}}</div>
          <div class="address-right">
            <div class="content"></div>
          </div>
        </div>

        <div class="pics-wrapper" v-if="sellerData.poi_env" ref="picsView">
          <ul class="pics-list" ref="picList">
            <li ref="picsItem"
                class="pics-item"
                v-for="(imgUrl, index) in sellerData.poi_env.thumbnails_url_list"
                :key="index">
              <img :src="imgUrl">
            </li>
          </ul>
      ...
      <div class="tip-wrapper">
        <div class="delivery-wrapper">
          配送服务: {{sellerData.app_delivery_tip}}
        </div>

        <div class="shipping-wrapper">
          配送时间: {{sellerData.shipping_time}}
        </div>
      </div>
      ...
      <div class="other-wrapper">
        <div class="server-wrapper">
          ...
          <div class="poi-server"
               v-for="(item,index) in sellerData.poi_service"
               :key="index">
        ...
        <div class="discounts-wrapper">
          <div class="discounts-item"
               v-for="(item,index) in sellerData.discounts2"
               :key="index">
            <div class="icon">
      ...
      fetch("/api/seller")
        ...
        .then(response => {
          ...
          this.$nextTick(() => {
            ...
            if (this.sellerData.poi_env.thumbnails_url_list)  {
              let imgW = this.$refs.picsItem[0].clientWidth;
              let marginR = 11;
              let width = (imgW + marginR) * this.sellerData.poi_env.thumbnails_url_list.length

              this.$refs.picList.style.width = width + "px";
            }
            if (!this.xScroll) {
              this.xScroll = new BScroll(this.$refs.picsView, {
                click: true,
                scrollX: true
              })
            } else {
